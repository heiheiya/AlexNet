...
def conv_layer(input, ksize, strides, name, b_value=0.0, padding='VALID', group=1):
    with tf.variable_scope(name) as scope:
        ...
        filter = tf.get_variable(name="weights", shape=[ksize[0], ksize[1], channels/group, ksize[3]], dtype=tf.float32, initializer=tf.truncated_normal(stddev=1e-1))
        biases = tf.get_variable(name="biases", shape=[ksize[3]], dtype=tf.float32, initializer=tf.constant_initializer(b_value))
...
def full_connected_layer(input, n_out, name, b_value =0.0, relu=True):
    ...
    with tf.variable_scope(name) as scope:
        weights = tf.get_variable(name="weights", shape=[n_in, n_out], dtype=tf.float32, initializer=tf.truncated_normal(stddev=1e-1))
        biases = tf.get_variable(name="biases", shape=[n_out], dtype=tf.float32, initializer=tf.constant_initializer(b_value))
        if relu == True:
